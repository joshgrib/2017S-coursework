  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        (constr-exp (id args)
            (tu-val id (map (lambda (a) (value-of a env)) args)))

        (type-exp (id conss paramss)
            (unit-val))
        ...
        (case-exp (e bs)
          (let ((v1 (value-of e env))
            ...
            (cases expval v1
              (tu-val (t_id t_args)
                (cases caseBranch b1
                  (a-branch (b_id b_args b_exp)
                    (if (eq? b_id t_id)
                      ;;equal - evaluate b_exp with t_args mapped to b_args
                      (value-of b_exp (extend-env-lsts b_args t_args env))
                      (value-of (case-exp e (cdr bs)) env)))
                  (else (eopl:error "Improper input for case-exp, expected caseBranch"))))
              (else (eopl:error "Bad input for case-exp, expected tu-val")))))
